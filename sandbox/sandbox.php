...
<head>
    ...
    <title>Card Grid with Hover Effects</title>
...
<body>
  ...
  <script>
    let cardsHtml = '';
    let lotteryArrays = <?php  echo json_encode(LOTTERIES_INFO); ?>;
    let lotteryNames = new Set();
    for (let time in lotteryArrays){
        const lottery = lotteryArrays[time];
        lottery.forEach(lottery => {
            console.log(lottery.lottery_name);
        if(!lotteryNames.has(lottery.lottery_name)){

        console.log(`Lottery Name: ${lottery.lottery_name}`);
        console.log(`Lottery Type: ${lottery.lottery_type}`);
        console.log(`Link URL: ${lottery.link_url}`);
        console.log(`Period: ${lottery.period}`);
        console.log(`Start-End: ${lottery.start_end}`);
        console.log(`Num Mins Per Period: ${lottery.num_mins_per_period}`);
        console.log(`Country: ${lottery.country}`);
        console.log(`Timezone: ${lottery.timezone}`);
        console.log(`Table Name: ${lottery.table_name}`);
        console.log('---------------------');
        lotteryNames.add(lottery.lottery_name);
         cardsHtml += getCardsHtml(lottery.lottery_name);
        }
   ...
   console.log(lotteryNames);
    ...
    function getCardsHtml(lotteryName){
        return ` <a href="../utils/entry.php?tbl=${lotteryName}" target="_blank" style="text-decoration: none;"><div class="card" >
            <div class="img">
                <div class="save">
